;;; mido-mode.el --- Major mode for editing typescript
...
;; This is based on Karl Landstrom's barebones mido-mode. This
...
(defconst typescript--font-lock-keywords-1
  ...
  "Level one font lock keywords for `mido-mode'.")
...
(defconst typescript--font-lock-keywords-2
  ...
  "Level two font lock keywords for `mido-mode'.")
...
(defcustom typescript-indent-level 4
  "Number of spaces for each indentation step in `mido-mode'."
  :type 'integer
  :safe 'integerp
  :group 'typescript)
...
(defcustom mido-mode-hook nil
  "*Hook called by `mido-mode'."
  :type 'hook
  :group 'typescript)
...
(defvar mido-mode-syntax-table
  (let ((table (make-syntax-table)))
    (c-populate-syntax-table table)
    (modify-syntax-entry ?$ "_" table)
    (modify-syntax-entry ?` "\"" table)
    table)
  "Syntax table for `mido-mode'.")

(defvar typescript--quick-match-re nil
  "Autogenerated regexp used by `mido-mode' to match buffer constructs.")

(defvar typescript--quick-match-re-func nil
  "Autogenerated regexp used by `mido-mode' to match constructs and functions.")
...
(defvar typescript--cache-end 1
  "Last valid buffer position for the `mido-mode' function cache.")
...
(defun typescript--update-quick-match-re ()
  "Internal function used by `mido-mode' for caching buffer constructs.
...
(defun typescript-beginning-of-defun (&optional arg)
  "Value of `beginning-of-defun-function' for `mido-mode'."
  (setq arg (or arg 1))
...
(defun typescript--flush-caches (&optional beg ignored)
  "Flush the `mido-mode' syntax cache after position BEG.
...
(defun typescript-end-of-defun (&optional arg)
  "Value of `end-of-defun-function' for `mido-mode'."
  (setq arg (or arg 1))
...
(defun typescript--backward-syntactic-ws (&optional lim)
  "Simple implementation of `c-backward-syntactic-ws' for `mido-mode'."
  (save-restriction
...
(defun typescript--forward-syntactic-ws (&optional lim)
  "Simple implementation of `c-forward-syntactic-ws' for `mido-mode'."
  (save-restriction
  ...
  `(
    ...
    ,(list
      ...
      (list typescript--name-re
            ...
            '(end-of-line)
            '(0 font-lock-variable-name-face))))
  "Level three font lock for `mido-mode'.")
...
(defun typescript--class-decl-matcher (limit)
  "Font lock function used by `mido-mode'.
...
(defconst typescript--font-lock-keywords
  ...
  "Font lock keywords for `mido-mode'.  See `font-lock-keywords'.")
...
;;;###autoload
(define-derived-mode mido-mode prog-mode "midomode"
...
\\{mido-mode-map}"
  ...
  :group 'typescript
  :syntax-table mido-mode-syntax-table
...
(put 'mido-mode 'flyspell-mode-predicate
     'typescript--flyspell-mode-predicate)

;;;###autoload
(eval-after-load 'folding
  '(when (fboundp 'folding-add-to-marks-list)
     (folding-add-to-marks-list 'mido-mode "// {{{" "// }}}" )))

;;;###autoload
(add-to-list 'auto-mode-alist '("\\.mido$" . mido-mode))

(provide 'mido-mode)

;;; mido-mode.el ends here
